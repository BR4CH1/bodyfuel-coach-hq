  ...
  const extend = useMutation({
    ...
    onSuccess: (res: any, vars) => {
      if (res?.already_extended) {
        ...
        toast.success(
          `Trainingsplan um ${vars.weeks} ${vars.weeks === 1 ? "Woche" : "Wochen"} verlängert – Übungen wurden übernommen.`,
        );
      }
      invalidate();
    },
    ...
    >
      ...
      {plan && (
        <>
          ...
          {props.onExtend && (
            <div className="mt-3 rounded-lg border border-border bg-background/60 px-2.5 py-2">
              {!extendOpen ? (
                <button
                  type="button"
                  onClick={() => setExtendOpen(true)}
                  className="inline-flex min-h-[40px] w-full items-center justify-center gap-1.5 rounded-md border border-dashed border-border bg-background px-2.5 py-2 text-xs font-semibold hover:bg-accent"
                >
                  <Calendar className="h-3.5 w-3.5" /> + Wochen verlängern
                </button>
              ) : (
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-2">
                    {[1, 2, 4].map((w) => (
                      ...
                      >
                        {props.extendPending ? "Verlängere…" : `+${w} Woche${w > 1 ? "n" : ""}`}
                      </button>
                    ))}
                  </div>
                  <button
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Übungen der bestehenden Wochen werden 1:1 in die neuen Wochen kopiert. Status bleibt unverändert.
                  </p>
                </div>
              )}
            </div>
