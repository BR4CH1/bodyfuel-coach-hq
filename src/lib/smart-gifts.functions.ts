...
export const createGiftCode = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { label?: string; days?: number; max_uses?: number; expires_at?: string | null }) =>
    z.object({
...
export const validateGiftCode = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: row } = await supabaseAdmin
      .from("smart_gift_codes")
      .select("code, label, days, max_uses, uses, expires_at")
