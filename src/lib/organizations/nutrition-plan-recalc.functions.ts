 ...
 *  - Bereits getrackte Mahlzeiten (food_entries im gleichen Slot) werden NIE ersetzt.
 *  - `is_locked` (Coach-Sperre) Mahlzeiten werden NIE ersetzt.
 *  - Manuelle `nutrition_plan_meal_overrides` (Coach/Athlet, source != 'auto_load_recalc')
 *    werden NIE ersetzt.
 *  - Nur die verbleibenden offenen Meals werden proportional an die
 *    Belastungsstufe angepasst — als datumsgebundenes Override, nicht als
 *    Änderung an der Baseline-Mahlzeile. Beim nächsten Recalc kann so ohne
 *    Compound zurückgesetzt werden.
 */
...
const AUTO_SOURCE = "auto_load_recalc";

/** Load-Faktor 0..5. 3 = neutral. */
...
async function assertCallerIsOrgStaffOrSelf(
  ...
  args: { userId: string; orgId: string; targets: string[] },
): Promise<void> {
  // Self-Fall: Athlet triggert Recalc nur für sich selbst.
  if (args.targets.length === 1 && args.targets[0] === args.userId) return;
...
async function writeAutoOverrides(
  ...
  // Vorherige Auto-Overrides für diesen (user, date) entfernen (nur unsere Quelle).
  await admin
    .from("nutrition_plan_meal_overrides")
    .delete()
    .eq("user_id", args.userId)
 ...
 * aufrufbar. Verteilt Kalorien/Makros ausschließlich auf `open_meal_ids`
 * per date-scoped Meal-Override; Baseline-Mahlzeit bleibt unverändert.
 *
 * `userId` (optional) begrenzt den Recalc auf einen Athleten (z. B. bei
 * Athleten-Override oder Team-Match-Kontext für einen Spieler).
 */
export const recalcNutritionForDirtyDays = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await assertCallerIsOrgStaffOrSelf(context.supabase, {
    ...
    if (users.length === 0 || dates.length === 0) {
      return { affected_users: 0, reports: [] as DirtyDayReport[] };
    }
    ...
    for (const userId of users) {
      for (const date of dates) {
        ...
        if (
          ...
          // Kein offenes Meal — vorsorglich alte Auto-Overrides entfernen.
          await supabaseAdmin
            .from("nutrition_plan_meal_overrides")
            .delete()
            .eq("user_id", userId)
