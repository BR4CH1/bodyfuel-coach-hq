...
function NutritionContent() {
  ...
  // Load plans for the effective client
  useEffect(() => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const path = `${effectiveClientId}/${Date.now()}-${file.name.replace(/[^a-z0-9.\-_]/gi, "_")}`;
  ...
  // Demo-only mode (no Supabase login)
  if (!supabaseUser) {
    return (
        ...
        <div className="flex items-start gap-3 rounded-2xl border border-gold/40 bg-accent/20 p-5">
          ...
          <div className="text-sm">
            ...
            <p className="mt-1 text-muted-foreground">
              Mit einem echten Account kann dein Coach dir Ernährungspläne als PDF hochladen — du
              kannst sie sicher abrufen und archivieren.
            </p>
            <a
              href="/auth"
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-3xl font-bold sm:text-4xl">
            {isCoach ? "Pläne verwalten" : "Dein Plan"}
          </h1>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-gold/40 bg-card p-5">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]">
            <input
              ...
              placeholder="Titel (z.B. Cut Woche 1-4)"
              maxLength={120}
      ...
      {!loading && !current && !archive.length && (
        <div className="flex items-start gap-3 rounded-2xl border border-border bg-card p-5 text-sm text-muted-foreground">
          ...
          <span>
            {isCoach ? "Für diesen Kunden gibt es noch keine Pläne." : "Dein Coach hat noch keinen Plan hochgeladen."}
          </span>
        </div>
      )}
