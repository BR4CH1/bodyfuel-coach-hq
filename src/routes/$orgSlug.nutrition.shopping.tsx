...
export const Route = createFileRoute("/$orgSlug/nutrition/shopping")({
  head: () => ({ meta: [{ title: "Einkaufsliste — BODYFUEL" }] }),
  component: OrgNutritionShopping,
});

function OrgNutritionShopping() {
  ...
  return (
    <OrgAthleteLayout slug={org.slug} features={(home?.features as any) ?? []} primaryColor={primary}>
      ...
      <main className="mx-auto max-w-md space-y-5 px-4 py-5">
        <ShoppingListContent backTo={`/${org.slug}/nutrition`} />
      </main>
    </OrgAthleteLayout>
  );
}