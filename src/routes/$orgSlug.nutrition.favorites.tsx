...
export const Route = createFileRoute("/$orgSlug/nutrition/favorites")({
  head: () => ({ meta: [{ title: "Favoriten — BODYFUEL" }] }),
  component: OrgNutritionFavorites,
});

function OrgNutritionFavorites() {
  ...
  return (
    <OrgAthleteLayout slug={org.slug} features={(home?.features as any) ?? []} primaryColor={primary}>
      <header className="px-5 py-6 text-white" style={{ background: `linear-gradient(135deg, ${primary} 0%, #000 100%)` }}>
        ...
        <h1 className="font-display text-2xl font-bold">Favoriten</h1>
      </header>
      <main className="mx-auto max-w-md space-y-5 px-4 py-5">
        <NutritionFavoritesContent backTo={`/${org.slug}/nutrition`} />
      </main>
    </OrgAthleteLayout>
  );
}