...
SET kcal_per_100g = round(kcal_per_100g * density_g_per_ml, 2),
    ...
    fiber_per_100g = CASE WHEN fiber_per_100g IS NULL THEN NULL ELSE round(fiber_per_100g * density_g_per_ml, 2) END,
    sugar_per_100g = CASE WHEN sugar_per_100g IS NULL THEN NULL ELSE round(sugar_per_100g * density_g_per_ml, 2) END,
    saturated_fat_per_100g = CASE WHEN saturated_fat_per_100g IS NULL THEN NULL ELSE round(saturated_fat_per_100g * density_g_per_ml, 2) END,
    salt_per_100g = CASE WHEN salt_per_100g IS NULL THEN NULL ELSE round(salt_per_100g * density_g_per_ml, 2) END,
    sodium_mg_per_100g = CASE WHEN sodium_mg_per_100g IS NULL THEN NULL ELSE round(sodium_mg_per_100g * density_g_per_ml, 2) END,
    alcohol_per_100g = CASE WHEN alcohol_per_100g IS NULL THEN NULL ELSE round(alcohol_per_100g * density_g_per_ml, 2) END,
    polyols_per_100g = CASE WHEN polyols_per_100g IS NULL THEN NULL ELSE round(polyols_per_100g * density_g_per_ml, 2) END,
    organic_acids_per_100g = CASE WHEN organic_acids_per_100g IS NULL THEN NULL ELSE round(organic_acids_per_100g * density_g_per_ml, 2) END,
    macro_reference_unit = 'ml'
WHERE macro_reference_unit = 'g'
...
BEGIN
  ...
  -- EU 1169/2011 Annex XIV: P/C 4, fat 9, fibre 2, alcohol 7,
  -- polyols 2.4 and organic acids 3 kcal/g. Fibre is never folded into carbs.
...
-- Replace the former 5%-Atwater auto-warning with the complete EU audit below.
UPDATE public.nutrition_foods
SET needs_review = false,
    review_reason = NULL
WHERE review_reason LIKE 'Makro-Kalorien-Abweichung %';

-- Audit every existing row now. Invalid rows stay in the table for review but leave search/Smart.
UPDATE public.nutrition_foods
SET audited_at = NULL;
...
-- Preserve the exact entered amount/unit on tracker rows; serving_g remains a legacy mass field.
...
-- Recreate the public projection and append the newly safe columns. Internal review reasons stay hidden.
...
-- Indexed/server-side search: no 1,000-row client scan. Every query stays in BodyFuel's DB.
