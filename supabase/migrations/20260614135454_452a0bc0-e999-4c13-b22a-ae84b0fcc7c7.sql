...
BEGIN
  ...
  IF v_goal_weight IS NOT NULL AND v_target_date IS NOT NULL AND v_target_date > current_date THEN
    ...
    v_kcal_t := round(v_tdee + v_kcal_delta);
    v_kcal_r := round(v_tdee + v_kcal_delta - 250);

    v_min_kcal := CASE lower(coalesce(v_gender,'')) WHEN 'female' THEN 1200 ELSE 1500 END;
    IF v_kcal_t < v_min_kcal THEN v_kcal_t := v_min_kcal; END IF;
    IF v_kcal_r < v_min_kcal - 100 THEN v_kcal_r := v_min_kcal - 100; END IF;
...
BEGIN
  IF TG_OP = 'INSERT'
     OR NEW.training_goal   IS DISTINCT FROM OLD.training_goal
     OR NEW.goal_weight_kg  IS DISTINCT FROM OLD.goal_weight_kg
     OR NEW.goal_target_date IS DISTINCT FROM OLD.goal_target_date
  THEN
...
-- Recompute everyone now so existing users get the new (un-forced) calculation
DO $$
DECLARE r record;
BEGIN
